...
//		StaticCache::on();
		...
		if (strpos(substr($data->content,0,5), "[")){
			$arr = [];
			$row = $this->getObj("row");
			$block = $this->getObj('block');
			$codeBlock = $this->getObj("textAreaBlock");
			$codeBlock->data = $data->content;
	...
	public function printBuilderContent($element)
	{
		...
		foreach ($element as $item) {
			...
			if ($item->type == 'codeBlock' && !is_array($item->data)) {

				$str .= "<div class='blockElement'>" . html_entity_decode($item->data, ENT_QUOTES) . "</div>";
			}
			if ($item->type == 'textAreaBlock' && !is_array($item->data)) {
				$str .= "<div class='blockElement'>" . $item->data . "</div>";
			}
